...
PacketError KermitPacket::send(int socket, PacketType type, const char* data,
    ...
    logger.print((char*)"entered send()\n");
    // cerr << "entered send()\n";
    ...
    while (true) {
        ...
        if (ret == no_error) {
            ...
            if (ret == no_error) {
                ...
                logger.print(
                    (char*)"didn't receive any response, trying again...\n");
            ...
            if (ret == no_error) {
                ...
                if (ret == no_error) {
                    if (response.header.type == ack) {
                        ...
                        logger.print((
                            char*)"didn't receive ACK as response, trying "
                                  "again...\n");
                        // cerr << "didn't receive ACK as response, trying "
                        //         "again...\n";
                    }
    ...
    while (offset < data_size) {
        ...
        if (ret != no_error) {
            logger.print((char*)"errror when writing data to buffer\n");
        ...
        while (true) {
            int ret = packet.sendPacket(socket);
            if (ret == send_error) {
                logger.print((char*)"error when sending message\n");
            ...
            while (true) {
                ...
                if (ret == recv_timeout) {
                    ...
                    logger.print((char*)"received a kermit message\n");
            ...
            if (ret == no_error) {
                if (response.header.type == ack) {
                    logger.printColor(color::green, "received ACK\n");
                    sequence = (sequence + 1) %
                               64;  // 8 because sequence field has 6 bits
                    offset += size;
                } else if (response.header.type == nack) {
                    logger.printColor(color::red, "received NACK");
    ...
    logger.print("entire message sent, exiting send()\n");
...
PacketError KermitPacket::confirmSend(int socket) {
    logger.print("entering confirmSend()\n");
    // cerr << "entering confirmSend()\n";
    KermitPacket end = KermitPacket(end_transmission, 0);
    end.setCRC();
    while (true) {
        logger.print("sending end_transmission\n");
    ...
    logger.print("successfully sent end_transmission, exiting confirmSend()\n");
...
PacketType KermitPacket::receive(int socket, std::vector<char>* buffer) {
    ...
    while (true) {
        ...
        if (ret == PacketError::no_error) {
            if (packet.header.type == initialize) {
                logger.printColor(color::cyan, "received initialize\n");
    ...
    logger.printColor(color::cyan, "Sequence (%d) received", sequence);
    // cerr << "Sequence " << sequence << " Received, ";
    logger.print("Inserting data to buffer\n");
    ...
    while (true) {
        ...
        if (ret == no_error) {
            ...
            if (packet.header.type == end_transmission) {
                logger.printColor(color::blue, "END TRANSMISSION\n");
            ...
            // Wrong Type of Message
                ...
                logger.printColor(color::blue, "Wrong type\n");
            ...
            // Next Sequential Message
                ...
                logger.printColor(color::blue, "RECEIVED NEXT SEQUENCE (%d)\n",
                                  sequence);
            ...
            // Wrong Sequential Message
                ...
                logger.printColor(color::blue,
                             "WRONG SEQUENCE: expected (%d), but got (%d)\n",
                             sequence, (unsigned char)packet.header.sequence);
                // cerr << color::blue << "WRONG SEQUENCE: expected (" << sequence
            ...
            // Same message
                ...
                logger.printColor(color::blue, "RECEIVED SAME MESSAGE\n");
                logger.print("Sequence (%d) received\n", sequence);
...
void KermitPacket::printData() {
    ...
    for (int i = 0; i < this->header.size; i++) {
        // cerr << std::hex << this->data[i];
        // fprintf(stderr, "%02x", (unsigned char)this->data[i]);
        logger.printColor(color::red, (char*)"%02x", (unsigned char)this->data[i]);
    }
    cerr << ")\n";
